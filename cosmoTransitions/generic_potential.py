...
class generic_potential():
    ...
    def derST(self, Tn, phases, start_phase, Vtot, gradV):
        """
               Calculates d(S/T)/dT
        """
        temperature = [t for t in range(int(Tn - 50), int(Tn + 50)) if t >= 5]
        tmplst = []
        # get the S/T for the new T values
        for y in temperature:
            tmp = transitionFinder._tunnelFromPhaseAtT(
                y, phases, start_phase, Vtot, gradV, phitol=1e-7,
        ...
        # prepares the data for ploting
        xlist = [t[0] for t in tmplst]
        ylist = [t[1] for t in tmplst]

        # interpolate & get the derivative of the interpolation
        spl = interpolate.UnivariateSpline(xlist, ylist, k=2)
        splx = np.linspace(xlist[0], xlist[-1], 100)
        plt.clf()
        spl.set_smoothing_factor(50000)
        der_spl = spl.derivative()
        plt.figure(figsize=(8, 6))
        plt.plot(xlist, ylist, "ro", ms=5)
        plt.plot(splx, spl(splx), "b")
        plt.legend(['data points', 'fit'])
        plt.title(r"$S_3(T)/T$ plot")
        plt.tick_params(axis='y', right=True, direction='in', which='both')
        plt.ylabel(r"$S_3(T)$/T")
        plt.xlabel(r"Temperature[GeV]")
        plt.savefig("CosmoBetaU1.png")
        plt.show()
        return der_spl(Tn)
    ...
    def findAllTransitions(self, tunnelFromPhase_args={}):
        ...
        if self.phases is None:
            self.getPhases()
        self.TnTrans, self.ncritdict = transitionFinder.findAllTransitions(
