...
def tunnelFromPhase(phases, start_phase, V, dV, Tmax,
    ...
    try:
        if Tmin == 0.0:
            Tnuc = optimize.brentq(_tunnelFromPhaseAtT, 2.0, Tmax, args=args,
                            xtol=Ttol*10, maxiter=maxiter, disp=False)
        else:
        ...
        if nuclCriterion(outdict[Tmax]['action'], Tmax) > 0:
            ...
            Tnuc = Tmax
    rdict = outdict[Tnuc]
    return rdict, outdict if rdict['trantype'] > 0 else None
...
def findAllTransitions(phases, V, dV, tunnelFromPhase_args={}):
    ...
    while start_phase is not None:
        ...
        try:
            trans, odict = tunnelFromPhase(phases, start_phase, V, dV, Tmax,
        ...
        if trans is None and not start_phase.low_trans:
            ...
            Tmax = trans['Tnuc']
            STtmp = [[key, odict[key]['action']] for key in odict.keys()]
            ncritdict.append(STtmp)
        return transitions, ncritdict
    return transitions
